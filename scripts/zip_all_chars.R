###########################################
# zip_all_chars.R
# This script assembles zip code level covariates
###########################################
...
#1. WFH exposure from lodes
#2. pop density from census
#3. CBDs from cennsus
...
cbsa_name <- read_csv('./data/external_data/nhgis/nhgis0003_ds244_20195_2019_cbsa.csv', locale = locale(encoding = "UTF-8"))%>%
  select(CBSAA, NAME_E, ALUBE001) %>%
  rename(MsaName = NAME_E, CBSA = CBSAA, `2019 CBSA Population` = ALUBE001) %>%
  mutate(
    MsaName = iconv(MsaName, to = "ASCII//TRANSLIT", sub = ""),
    MsaName = str_replace_all(MsaName, "[^[:alnum:], -]", "")
  ) %>%
...
wfh_exp <- dens %>%
  ...
  left_join(wfh, by = 'zip') %>%
  left_join(zbp, by = 'zip') %>%
