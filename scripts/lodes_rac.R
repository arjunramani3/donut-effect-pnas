#######################################
# This script calculates the share of residents in each zip code 
...
tract_to_zip <- read_csv('./data/external_data/TRACT_ZIP_092020.csv') %>%
...
lodes_naics <- read_csv('./data/external_data/LODES-code-NAICS.csv') %>%
...
for (code in states$state_abbr) {
  ...
  cur <- read_csv(path) %>% mutate(tract = substr(h_geocode, 0, 11)) %>% 
    ...
    inner_join(lodes_naics, by = 'Variable')
  #merge with crosswalk and aggregate to zip code level
  cur <- cur %>% left_join(tract_to_zip, by = 'tract') %>%
  ...
  cur <- cur %>% left_join(wfh_exposure, by = 'NAICS') %>%
...
write_csv(all2, './data/lodes_rac_wfh_zip_exposure.csv')
