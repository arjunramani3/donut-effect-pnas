...
start_date = '2019-03-01' #start period of cumulation ending in 2020-03-01 exclusive
end_date = '2023-03-01' #end period for cumulation starting in 2020-03-01 inclusive, for regressions
end_date_maps = '2023-07-01' #end period for cumulation starting in 2020-03-01 inclusive, for heat maps

USPS_names = c('date', 'zip', 'CITY', 'STATE', 
               'TOTAL FROM ZIP', 'TOTAL BUSINESS', 'TOTAL FAMILY', 'TOTAL INDIVIDUAL', 'TOTAL PERM', 'TOTAL TEMP',
               'TOTAL TO ZIP', 'TOTAL BUSINESS_1', 'TOTAL FAMILY_1', 'TOTAL INDIVIDUAL_1', 'TOTAL PERM_1', 'TOTAL TEMP_1')

## Create function to read USPS data
read_USPS <- function(path) {
  df <- read_csv(path, 
                 col_names = USPS_names,
                 col_types = 'ccccdddddddddddd',
                 skip = 1) %>%
    mutate(
      zip = gsub("=", "", zip),
      zip = gsub("\"", "", zip),
...
## Read in USPS data
## Please download from the USPS frequently requested FOIA webpage: https://about.usps.com/who/legal/foia/library.htm
## Data not stored in the github repository due to large file size
df17 <- read_USPS('./data/external_data/USPS_online/Y2017.csv')
df18 <- read_USPS('./data/external_data/USPS_online/Y2018.csv')
df19 <- read_USPS('./data/external_data/USPS_online/Y2019.csv')
df20 <- read_USPS('./data/external_data/USPS_online/Y2020.csv')
df21 <- read_USPS('./data/external_data/USPS_online/Y2021.csv')
df22 <- read_USPS('./data/external_data/USPS_online/Y2022.csv')
df23 <- read_USPS('./data/external_data/USPS_online/Y2023.csv')

df_all <- rbind(df17, df18, df19, df20, df21, df22, df23)
...
###########################################
# Create zip pct change file from panel, ending in Feb 2023 for regressions
###########################################
...
###########################################
# Create zip pct change file from panel, ending in June 2023 for heat maps
###########################################
chars <- read_csv('./data/zip_all_chars_cbd.csv', 
                  col_types = cols('zip' = col_integer())) %>% dplyr::select(!estab_count)
bus_chars <- read_csv('./data/zbp_wfh.csv',
...
## Get post-period percent growth
df3 <- df_all %>%
  filter(date >= as.Date('2020-03-01'), date < as.Date(end_date_maps)) %>%
  group_by(zip) %>% summarise(
    post_net = sum(net, na.rm = TRUE),
    post_bus = sum(net_bus, na.rm = TRUE),
    post_pop = sum(net_pop, na.rm = TRUE),
...
df6 <- df3 %>% dplyr::select(zip, post_net, post_bus, post_pop, post_temp, post_perm) %>% inner_join(df4, by = 'zip') %>%
  filter(!is.na(dist_to_cbd)) %>%
  write_csv('./data/usps_panel_zips_maps.csv')

###########################################
# Create MSA pct change file from panel, ending in Feb 2023
###########################################
